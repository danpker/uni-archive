 ...
 *
 * neighbour_joining.cu - neighbour joining
 */
...
__global__ void calcDistanceMatrix(char *taxa, int size, int len, int *dmat) {

	__shared__ int add[LEN];

    int i = blockIdx.x;
    int j = blockIdx.y;
    int k = threadIdx.x;

    if (taxa[i * LEN + k] != taxa[j * LEN + k]) {
     	add[k] = 1;
    } else {
     	add[k] = 0;
    }
...
void createDistanceMatrix(char *taxa[], int size, int len, NJDMAT *matrix) {
	...
	HANDLE_ERROR ( cudaMalloc( (void **) &dev_d, size * size * sizeof(int)) );
	HANDLE_ERROR ( cudaMemset( dev_d, 0, size * size * sizeof(int)));
	...
	for (i = 0; i < SIZE; i++) {
		for (j = 0; j < LEN; j++) {
			if(taxa[i][j] != '\0') {
				newtaxa[i * LEN + j] = taxa[i][j];
			}
			
		}
	}
	printf("\n");
	HANDLE_ERROR( cudaMalloc( (void **) &dev_taxa, SIZE * LEN * sizeof(char)) );
	HANDLE_ERROR( cudaMemcpy( dev_taxa, newtaxa, SIZE * LEN * sizeof(char), cudaMemcpyHostToDevice) );

	dim3 mat(SIZE, SIZE);
	dim3 threads(LEN);
	calcDistanceMatrix<<<mat, threads>>>(dev_taxa, size, len, dev_d);

	HANDLE_ERROR ( cudaMemcpy(d, dev_d, size * size * sizeof(int), cudaMemcpyDeviceToHost) );

	for (i = 0; i < size; i++) {
		for(j = 0; j < size; j++) {
			//printf("%3d ", d[i * size + j]);
			matrix->distances[i * size + j] = d[i * size + j];
		}
	}
